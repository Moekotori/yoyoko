...
The add-instance overlay focuses the address field, restores prior focus on close, blocks background shortcuts, and closes after an in-flight connection is cancelled. Closing search returns focus to the composer; closing the quick switcher restores its previous focus. Escape inside settings is left to the focused control first, allowing an open control to dismiss before the entire settings page closes. Search filters only the local timeline window, not all server history.
...
## Chat-facing language (2026-09-23)

The normal chat surface uses shorter labels: “参与者” instead of “本页参与者,” “查找消息” instead of “搜索已加载消息,” and a scoped empty result that says it searched the messages currently shown. With no selected server, the title uses the configured product name instead of “工作空间.” User cards and the own-profile drawer keep copy-ID actions but no longer put a UUID in the primary card layout. The community menu presents a copy-invite-link action instead of a raw invite code and no longer repeats the server URL; address and protocol details remain in Server settings.

The composer now shows only message, attachment, and history errors. Connection status stays on the connection page; other failures appear as a dismissible notice outside the composer. Common connection, timeout, permission, missing-content, and sign-in errors use localized, user-readable text instead of raw transport or API messages; unexpected errors retain their detailed message. A focused headless check verified this status separation and rendered the production chat/card controls with local sample data. Release client build passed; native pointer/keyboard and server-backed send behavior were not rechecked in this copy pass.

## Whole UI polish (2026-09-23)

At a narrow chat width, the optional 248px participant list becomes a right-side drawer with a dismissible scrim, so opening it no longer shrinks the message timeline or composer. General settings keep language and the memory-saving switch visible; language-pack import/export is available behind a keyboard-accessible disclosure and opens automatically for a JSON file drag. Server settings keep connection state and name visible while host, latency, and protocol move under “Connection details”; inviting uses a copy-link action rather than displaying the code. Uncustomized profile banners use restrained neutral gray swatches instead of saturated random colors. The disconnected screen now gives Connect a primary button and Settings a secondary button.

Release client build passed with zero warnings/errors. Production controls were rendered headlessly at 1200×820 and 980×640 in dark mode, plus narrow light chat/settings and both expanded detail sections. This checks layout and theme resources, not native OS pointer/keyboard input, network acceptance, hardware, or server-backed chat actions.
...
- General: language and the memory-saving switch are primary rows. Language-pack tools stay in a collapsed section until opened or a JSON file is dragged in. Two-way language selection updates the existing preferences and section title.
...
- Server: the address field accepts an origin or a shareable `{origin}/join/{invite_code}` link. After a session is up, the page shows connection state, server and community names, and a copy-invite-link action. Host, latency, and protocol stay available under Connection details. Disconnect stops Gateway/voice for that instance and keeps the saved account so Connect can restore it. Changing the address while connected shows Connect again to switch.
...
- `Chat/UserCard`: the message/member flyout is 300px wide with a 104px banner and 66px avatar. The long user ID is no longer shown by default; a copy action remains beside the handle. Presence text is hidden when no live status is known. `Chat/ProfileDrawer`: the account-bar profile remains a separate 360px popout above the account strip, with its ID available through a copy action. Full profile editing in Settings is unchanged.
...
频道/账号列为 236 px，给底部头像、用户名和麦克风/耳机控件留出间距。已实现紧凑频道分组、2 px 选中行圆角和分组旁的新增按钮。所有者可添加文字/语音频道，文字频道右键打开、重命名、添加同类频道或删除。语音频道单击加入并打开该频道的文字聊天；右键可复制网页语音链接，或打开语音频道设置弹窗查看名称和频道音质上限（所有者可修改）。个人语音设备和音质仍在个人设置页。成员列在该语音频道下方，账号条上方可离开。添加与重命名直接在侧栏原位输入，Enter 保存、Esc 取消；删除在原行展开确认，失败就地显示错误，创建语音频道不会自动加入通话。管理请求走 HTTP，变更通过 Gateway 与 SQLite 同步。

当前成员栏默认收起，仅汇集已加载的真实消息作者和当前账号，标题为「参与者」；服务端完整成员名单与 presence **尚未实现**，未知状态不标记为离线。窄窗口中成员栏以带遮罩的抽屉打开，点遮罩可关闭，不再挤压消息正文。正常启动不注入假频道、假成员或假消息；仅显式 `--design-preview` 使用本地布局样例。没有头像时显示名字首字；左键打开紧凑用户卡片，ID 可复制但不常驻展示；右键可发私信、提及或复制。真实账号可打开 1:1 私信，所有者的踢出/封禁入口仍明确提示尚未实现。
