...
| 设置页面 | 左下角入口，打开时频道列收起；分类含通用、外观、快捷键、语音、资料和服务器。语言包与连接技术详情按需展开，深浅主题、背景、动效和设备设置保持独立；[界面验证记录](docs/ui/README.md#whole-ui-polish-2026-09-23) |
| 界面细节 | 聊天成员栏在窄窗口改为可关闭抽屉，离线连接页突出主要操作，默认资料横幅使用中性色；[本轮视觉与验证](docs/ui/README.md#whole-ui-polish-2026-09-23) |
...
| 实例发现、添加、SQLite 保存及离线恢复 | 已实现；实例栏加号打开独立服务器弹窗，填写地址与可选服务器密码；实例发现或 Gateway 连接失败时头像角落显示告警，Gateway 恢复后消失；本轮弹窗与密码接入待真实双端验证 |
...
| 界面内存调度 | 已接入前台 / 空闲 / 压力 / 隐藏四档；设置 → 通用可开启“最小化时节省内存”，最小化/隐藏时卸载页面，保留消息和语音会话，恢复失败可重试；[策略与验证](docs/MEMORY_SCHEDULING.md) |
...
保存的会话无法恢复时，客户端显示该实例的登录/注册表单；在添加服务器弹窗里重复输入同一地址也会转到此表单。客户端不会静默创建新账号或清除旧账号缓存。若服务端换了数据库，原账号和消息需要恢复原服务端数据才能找回；在新数据库注册会创建独立的新账号。
...
实例栏的 `L` 代表本机 `localhost:8080` 服务端，不是仅保存在客户端的离线频道。本机首次连接会自动建立实例账号；会话异常时显示重新连接，已有账号登录收在手动入口，避免启动时直接展示登录/注册表单。已有账号会话失效不会自动更换身份或删除本地缓存。
