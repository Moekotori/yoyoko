...
public sealed class TextCatalog : ITextCatalog
{
    ...
    [
        ...
        (TextKey.UltraLightMode, "Save memory when minimized", "最小化时节省内存", "最小化時にメモリを節約"),
        ...
        ("CopyWebVoiceLink", "Copy web voice link", "复制网页语音链接", "ウェブボイスリンクをコピー"),
        ("WebVoiceLinkCopied", "Web voice link copied", "已复制网页语音链接", "ウェブボイスリンクをコピーしました"),
        ...
        (TextKey.ConnectionDetails, "Connection details", "连接详情", "接続の詳細"),
        ...
        ("CopyInvite", "Copy invite link", "复制邀请链接", "招待リンクをコピー"),
        ("SessionRecoveryRequired", "The saved account could not be restored. Check the server data or use an existing account.", "已保存的账号无法恢复，请检查服务端数据或使用已有账号。", "保存済みのアカウントを復元できません。サーバーデータを確認するか、既存のアカウントを使用してください。"),
        ("LocalConnectionTitle", "Local community", "本机社区", "ローカルコミュニティ"),
        ("RetryLocalConnection", "Retry connection", "重新连接", "再接続"),
        ("UseExistingAccount", "Use existing account", "使用已有账号", "既存のアカウントを使用"),
        ("BackToLocalConnection", "Back", "返回", "戻る"),
        ("PageParticipants", "People here", "参与者", "参加者"),
        ...
        ("OptionalServerPassword", "Server password (optional)", "服务器密码（可留空）", "サーバーパスワード（任意）"),
        ("InvalidServerPassword", "Server password is required or incorrect.", "服务器密码缺失或不正确。", "サーバーパスワードが必要か、正しくありません。"),
        ...
        ("DefaultUsername", "User", "User", "User"),
        ("JoinUsernameHint", "Enter a username for this server. Leave empty to use User.", "加入此服务器时请填写用户名，不填则默认为 User。", "このサーバーのユーザー名を入力してください。空欄の場合は User になります。"),
        ...
        (TextKey.CancelSend, "Cancel send", "取消发送", "送信をキャンセル"),
        (TextKey.OutboxFull, "Too many unsent messages. Wait or cancel one.", "未发送消息过多，请等待或取消一条。", "未送信のメッセージが多すぎます。待つか、1件キャンセルしてください。"),
        ("Image", "Image", "图片", "画像"),
        ...
        ("VoiceJoinHint", "Click a voice channel to join and open its text chat", "点击语音频道加入并打开文字聊天", "ボイスチャンネルをクリックして参加し、テキストチャットを開きます"),
        ("VoiceMediaError", "In the channel, audio not connected: {0}", "已进入频道，音频未接通：{0}", "チャンネルに入りましたが、音声は未接続です：{0}"),
        ("VoiceReconnecting", "Reconnecting audio…", "正在重新连接音频…", "音声を再接続しています…"),
        ...
        (TextKey.ServerUnreachable, "Can’t reach this server right now. Try again in a moment.", "暂时连不上这台服务器，请稍后重试。", "サーバーに接続できません。少し待ってから再試行してください。"),
        (TextKey.RequestTimedOut, "The server took too long to respond. Try again.", "服务器响应太慢，请重试。", "サーバーの応答が遅れています。もう一度お試しください。"),
        (TextKey.ActionNotAllowed, "You don’t have permission to do that.", "你没有权限进行此操作。", "この操作を行う権限がありません。"),
        (TextKey.ContentUnavailable, "This content is no longer available.", "这项内容已经无法使用。", "この内容は利用できなくなりました。"),
        (TextKey.SignInAgain, "Sign in again to continue.", "请重新登录后继续。", "続けるには再ログインしてください。"),
        ...
        ("NoMatchingMessages", "Nothing matched in the messages shown here", "当前显示的消息里没有找到", "表示中のメッセージに一致するものはありません"),
        ...
        ("OfflineCount", "Offline — {0}", "离线 — {0}", "オフライン — {0}"),
        ...
        ("SearchInChannel", "Find a message", "查找消息", "メッセージを探す"),
        ...
        ("CannotAttachWhileEditing", "Finish editing before attaching files", "请先结束消息编辑，再添加文件", "ファイルを添付する前に編集を完了してください"),
        ...
        ("ChangeBanner", "Change banner", "更换横幅", "バナーを変更"),
        ("RemoveBanner", "Remove banner", "移除横幅", "バナーを削除"),
        ("SaveProfile", "Save", "保存", "保存"),
        ("ProfileSaved", "Profile saved", "资料已保存", "プロフィールを保存しました"),
        ("PickAvatar", "Choose avatar", "选择头像", "アバターを選択"),
        ("PickBanner", "Choose banner", "选择横幅", "バナーを選択"),
        ...
        ("InvalidBanner", "Banner must be a JPEG, PNG, GIF, or WebP image.", "横幅需为 JPEG、PNG、GIF 或 WebP 图片。", "バナーは JPEG、PNG、GIF、または WebP 画像にしてください。"),
        ...
        ("MessageUser", "Message", "发私信", "メッセージ"),
        ("DirectMessages", "Direct messages", "私信", "ダイレクトメッセージ"),
        ("MessageToDirect", "Message {0}", "发消息给 {0}", "{0} に送信"),
        ("DirectUnavailable", "That name is only a layout placeholder.", "占位用户不能发私信。", "レイアウト用の名前には送れません。"),
        ("MentionEveryone", "Everyone", "所有人", "全員"),
        ("MentionHere", "Here", "在线成员", "オンライン"),
        ("MentionPicker", "Mention", "提及", "メンション"),
        ...
        ("DeleteMessage", "Delete message", "删除消息", "メッセージを削除"),
        ("Reply", "Reply", "回复", "返信"),
        ("CancelReply", "Cancel reply", "取消回复", "返信をキャンセル"),
        ("ReplyingTo", "Replying to {0}", "正在回复 {0}", "{0} に返信中"),
        ("ReplyQuote", "{0}: {1}", "{0}：{1}", "{0}：{1}"),
        ("ReplyMissing", "Original message", "原消息", "元のメッセージ"),
        ("TypingOne", "{0} is typing…", "{0} 正在输入…", "{0} が入力中…"),
        ("TypingTwo", "{0} and {1} are typing…", "{0}、{1} 正在输入…", "{0} と {1} が入力中…"),
        ("TypingMany", "{0} and {1} others are typing…", "{0} 等 {1} 人正在输入…", "{0} ほか {1} 人が入力中…"),
        ("ChannelForbidden", "You no longer have access to this channel.", "你已无权查看此频道。", "このチャンネルを見る権限がありません。"),
